...
$pdf->Image('../../style/assets/img/kabtegal.png',15,13,30);
$pdf->SetTitle("Cetak Data");
$pdf->SetFont('Arial','B',18);
$pdf->Cell(0,5,'SD N JATIRAWA 02 ','0','1','C',false);
$pdf->SetFont('Arial','i',8);
$pdf->Cell(0,5,'Alamat : Jln. Raya Jatirawa No. 1, Tarub, Tegal','0','1','C',false);
$pdf->Cell(0,2,'Telp : 0856-4267-3103 - Email : sdnjatirawa02@gmail.com','0','1','C',false);
$pdf->Ln(3);
...
if(@$_GET['data'] == "pengajar") {
	...
	$pdf->Cell(100,5,'Laporan Data Tenaga Pengajar','0','35','center',false);
	$pdf->Ln(5);

	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(10,6,'No.',1,0,'C');
	$pdf->Cell(33,6,'NIP',1,0,'C');
	$pdf->Cell(40,6,'Nama Lengkap',1,0,'C');
	$pdf->Cell(35,6,'TTL',1,0,'C');
	$pdf->Cell(10,6,'JK',1,0,'C');
	$pdf->Cell(12,6,'Agama',1,0,'C');
	...
	while($data = mysqli_fetch_array($sql)) {
		if($data['jenis_kelamin'] == 'L') { $jk = "L"; } else { $jk = "P"; }
		$pdf->Ln(4);
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(10,4,$no++.".",1,0,'C');
		$pdf->Cell(33,4,$data['nip'],1,0,'L');
		$pdf->Cell(40,4,$data['nama_lengkap'],1,0,'L');
		$pdf->Cell(35,4,$data['tempat_lahir'].", ".tgl_indo($data['tgl_lahir']),1,0,'L');
		$pdf->Cell(10,4,$jk,1,0,'C');
		$pdf->Cell(12,4,$data['agama'],1,0,'C');
		$pdf->Cell(15,4,$data['jabatan'],1,0,'L');
		$pdf->Cell(85,4,$data['alamat'],1,0,'L');
	...
	$pdf->Cell(10,6,'Agama',1,0,'C');
	$pdf->Cell(27,6,'Nama Ayah',1,0,'C');
	...
	while($data = mysqli_fetch_array($sql)) {
		...
		$pdf->Cell(10,4,$data['nis'],1,0,'L');
		$pdf->Cell(32,4,$data['nama_lengkap'],1,0,'L');
		$pdf->Cell(32,4,$data['tempat_lahir'].", ".tgl_indo($data['tgl_lahir']),1,0,'L');
		$pdf->Cell(8,4,$data['jenis_kelamin'],1,0,'C');
		$pdf->Cell(10,4,$data['agama'],1,0,'C');
		$pdf->Cell(27,4,$data['nama_ayah'],1,0,'L');
		$pdf->Cell(30,4,$data['nama_ibu'],1,0,'L');
	...
	$pdf->Cell(30,6,'TTL',1,0,'C');
	$pdf->Cell(10,6,'JK',1,0,'C');
	$pdf->Cell(13,6,'Agama',1,0,'C');
	$pdf->Cell(30,6,'Nama Ayah',1,0,'C');
	$pdf->Cell(30,6,'Nama Ibu',1,0,'C');
	$pdf->Cell(19,6,'No. Telepon',1,0,'C');
	$pdf->Cell(35,6,'Email',1,0,'C');
	$pdf->Cell(38,6,'Alamat',1,0,'C');
	...
	while($data = mysqli_fetch_array($sql)) {
		...
		$pdf->Cell(7,4,$no++.".",1,0,'C');
		$pdf->Cell(10,4,$data['nis'],1,0,'L');
		$pdf->Cell(35,4,$data['nama_lengkap'],1,0,'L');
		$pdf->Cell(30,4,$data['tempat_lahir'].", ".tgl_indo($data['tgl_lahir']),1,0,'L');
		$pdf->Cell(10,4,$data['jenis_kelamin'],1,0,'C');
		$pdf->Cell(13,4,$data['agama'],1,0,'C');
		$pdf->Cell(30,4,$data['nama_ayah'],1,0,'L');
		$pdf->Cell(30,4,$data['nama_ibu'],1,0,'L');
		$pdf->Cell(19,4,$data['no_telp'],1,0,'L');
		$pdf->Cell(35,4,$data['email'],1,0,'L');
		$pdf->Cell(38,4,$data['alamat'],1,0,'L');
	...
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(50,5,'Laporan Daftar Kelas, Letak Ruang, Wali dan Ketua Masing-masing Kelas di SD N Jatirawa 02','0','1','L',false);
	$pdf->Ln(3);

	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(10,6,'No.',1,0,'C');
	$pdf->Cell(50,6,'Nama Kelas',1,0,'C');
	$pdf->Cell(50,6,'Letak Ruang',1,0,'C');
	$pdf->Cell(70,6,'Wali Kelas',1,0,'C');
	$pdf->Cell(70,6,'Ketua Kelas',1,0,'C');
	$pdf->Ln(2);
	$no = 1;
	...
	while($data = mysqli_fetch_array($sql)) {
		...
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(10,4,$no++.".",1,0,'C');
		$pdf->Cell(50,4,$data['nama_kelas'],1,0,'C');
		$pdf->Cell(50,4,$data['ruang'],1,0,'C');
        ...
        if($cek_tampil_guru > 0) {
        	$pdf->Cell(70,4,$data_tampil_guru['nama_lengkap'],1,0,'L');
        } else {
            $pdf->Cell(70,4,"Belum diatur",1,0,'L');
        }
	    ...
	    if($cek_tampil_siswa > 0) {
	    	$pdf->Cell(70,4,$data_tampil_siswa['nama_lengkap'],1,0,'L');
	    } else {
	        $pdf->Cell(70,4,"Belum diatur",1,0,'L');
	    }
	}
	...
	while($data = mysqli_fetch_array($sql_materi)) {
		...
		if($data['pembuat'] == 'admin') {
			...
			$pdf->Cell(50,4,$data_pengajar['nama_lengkap'],1,0,'L');
		}
...
$pdf->Cell(0,0,"Tarub, ".tgl_indo(date('Y-m-d')),0,0,'L');
$pdf->Ln(20);
